			...
			<div class="col-md-12">
				<div class="card">
					...
					<div class="card-header">
						<strong class="card-title">UPDATE CALENDAR EVENT</strong>
					</div>
					<div class="card-body">
						<c:url var="loadCalendar" value="/updateCalendarEvent" />
						<mvc:form method="POST" action="${loadCalendar}"
							...
							<div class="row">
								...
								<input
										class="form-control" type="hidden" name="calendarID" id="calendarID"
										value="${response.calendarID}" path="calendarID"
										autocomplete="off">

								<div class="col-md-6">
									<label class="bmd-label-floating">Name*</label> <input
										class="form-control" type="text" name="createdBy" id="createdBy"
										value="${response.createdBy}" path="createdBy" required readonly
										autocomplete="off">
								</div>
							...
							<div class="row">
								...
								<div class="col-md-6">
									...
									<textarea class="form-control" type="text" name="event"
										id="event" path="event"
										required autocomplete="off">${response.event}</textarea>
								</div>
								
								<div class="col-md-6">
									<label class="bmd-label-floating">Event Date*</label> <input
										class="form-control" type="date" name="eventDate" id="eventDate"
										value="${response.eventDateD}" path="eventDate" required onchange="checkdate()"
										autocomplete="off">
								</div>
								...
								<div class=" col-md-6">
									<label for="disabledTextInput">Send To*</label> <select
										...
										<option value="${response.sendTo}">${response.sendTo}</option>
...
function checkdate(){
	...
	if((dateDate < todaysDate)){
	 /* alertjs.show({
        ...
        alert('Event Date cannot be less than today');
